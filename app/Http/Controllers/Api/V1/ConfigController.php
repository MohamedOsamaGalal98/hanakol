...
class ConfigController extends Controller
{
    ...
    public function place_api_autocomplete(Request $request)
    {
        ...
        $searchText = urlencode($request['search_text']);
        $url = 'https://maps.googleapis.com/maps/api/place/autocomplete/json?input=' . $searchText . '&key=' . $this->map_api_key;
    ...
    public function distance_api(Request $request)
    {
        ...
        $origin_lat = urlencode($request['origin_lat']);
        $origin_lng = urlencode($request['origin_lng']);
        $destination_lat = urlencode($request['destination_lat']);
        $destination_lng = urlencode($request['destination_lng']);

        $url = 'https://maps.googleapis.com/maps/api/distancematrix/json?origins=' . $origin_lat . ',' . $origin_lng . '&destinations=' . $destination_lat . $destination_lng . '&key=' . $this->map_api_key . '&mode=walking';
    ...
    public function place_api_details(Request $request)
    {
        ...
        $placeid = urlencode($request['placeid']);
        $url = 'https://maps.googleapis.com/maps/api/place/details/json?placeid=' . $placeid . '&key=' . $this->map_api_key;
    ...
    public function geocode_api(Request $request)
    {
        ...
        $lat = urlencode($request->lat);
        $lng = urlencode($request->lng);
        $url = 'https://maps.googleapis.com/maps/api/geocode/json?latlng=' . $lat . ',' . $lng . '&key=' . $this->map_api_key;
